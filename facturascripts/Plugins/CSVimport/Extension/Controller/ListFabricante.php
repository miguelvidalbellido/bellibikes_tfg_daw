...
use FacturaScripts\Plugins\CSVimport\Lib\ImportProfile\ManufacturersProfile;
...
class ListFabricante
{
    ...
    public function execPreviousAction(): Closure
    {
        return function ($action) {
            switch ($action) {
                case 'import-manufacturers':
                    $this->importManufacturersAction();
                    break;

                case 'upload-manufacturers':
                    $this->uploadManufacturersAction();
                    break;
            }
        };
    }

    public function importManufacturersAction(): Closure
    {
        return function () {
            // obtenemos la ruta completa del archivo
            $fileName = $this->request->get('import-filename');
            $filePath = ManufacturersProfile::getFilePath($fileName);
            if (empty($filePath)) {
                return true;
            }
            ...
            if ($template === 'new-template') {
                $newCsvFile = CSVfile::newTemplate($fileName, 'manufacturers');
            ...
            $templateModel = $template === 'automatic' ?
                ManufacturersProfile::getFileTemplate($filePath) :
                ManufacturersProfile::getFileTemplate($filePath, $template);
            ...
            if ($result['offset'] > 0 && $result['offset'] < $result['total']) {
                ...
                $this->redirect($this->url() . '?action=import-manufacturers&import-filename=' . $fileName . '&import-offset='
        ...
        return function ($viewName, $view) {
            ...
            if ($column && $column->widget->getType() === 'select') {
                ...
                $where = [
                    ...
                    new DataBaseWhere('profile', 'manufacturers')
                ];
    ...
    public function uploadManufacturersAction(): Closure
    {
        return function () {
            ...
            $this->redirect($this->url() . '?action=import-manufacturers&import-filename=' . urlencode($fileName)
