...
use FacturaScripts\Plugins\CSVimport\Lib\ImportProfile\CustomerDeliveryNotesProfile;
...
class ListAlbaranCliente
{
    ...
    public function importDeliveryNotesAction(): Closure
    {
        return function () {
            ...
            $filePath = CustomerDeliveryNotesProfile::getFilePath($fileName);
            if (empty($filePath)) {
                return true;
            }
            ...
            $templateModel = $template === 'automatic' ?
                CustomerDeliveryNotesProfile::getFileTemplate($filePath) :
                CustomerDeliveryNotesProfile::getFileTemplate($filePath, $template);
            ...
            if ($result['offset'] > 0 && $result['offset'] < $result['total']) {
                ...
                $this->redirect($this->url() . '?action=import-delivery-notes&import-filename=' . $fileName . '&import-offset='
