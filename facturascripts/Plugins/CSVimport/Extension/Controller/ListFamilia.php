...
use FacturaScripts\Plugins\CSVimport\Lib\ImportProfile\FamiliesProfile;
...
class ListFamilia
{
    ...
    public function execPreviousAction(): Closure
    {
        return function ($action) {
            switch ($action) {
                ...
                case 'upload-families':
                    $this->uploadFamiliesAction();
                    break;
            }
        };
    }

    public function importFamiliesAction(): Closure
    {
        return function () {
            // obtenemos la ruta completa del archivo
            $fileName = $this->request->get('import-filename');
            $filePath = FamiliesProfile::getFilePath($fileName);
            if (empty($filePath)) {
                return true;
            }
            ...
            if ($template === 'new-template') {
                $newCsvFile = CSVfile::newTemplate($fileName, 'families');
            ...
            $templateModel = $template === 'automatic' ?
                FamiliesProfile::getFileTemplate($filePath) :
                FamiliesProfile::getFileTemplate($filePath, $template);
            ...
            if ($result['offset'] > 0 && $result['offset'] < $result['total']) {
                ...
                $this->redirect($this->url() . '?action=import-families&import-filename=' . $fileName . '&import-offset='
        ...
        return function ($viewName, $view) {
            ...
            if ($column && $column->widget->getType() === 'select') {
                ...
                $where = [
                    ...
                    new DataBaseWhere('profile', 'families')
                ];
    ...
    public function uploadFamiliesAction(): Closure
    {
        return function () {
            ...
            $this->redirect($this->url() . '?action=import-families&import-filename=' . urlencode($fileName)
