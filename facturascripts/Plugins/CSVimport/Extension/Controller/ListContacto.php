...
use FacturaScripts\Plugins\CSVimport\Lib\ImportProfile\ContactsProfile;
...
class ListContacto
{
    public function createViews(): Closure
    {
        return function () {
            ...
            if ($this->permissions->allowImport) {
                $this->addButton('ListCrmContacto', [
        ...
        return function ($action) {
            switch ($action) {
                ...
                case 'upload-contacts':
                    $this->uploadContactsAction();
                    break;
            }
        };
    }

    public function importContactsAction(): Closure
    {
        return function () {
            // obtenemos la ruta completa del archivo
            $fileName = $this->request->get('import-filename');
            $filePath = ContactsProfile::getFilePath($fileName);
            if (empty($filePath)) {
                return true;
            }
            ...
            if ($template === 'new-template') {
                $newCsvFile = CSVfile::newTemplate($fileName, 'contacts');
            ...
            $templateModel = $template === 'automatic' ?
                ContactsProfile::getFileTemplate($filePath) :
                ContactsProfile::getFileTemplate($filePath, $template);
            ...
            if ($result['offset'] > 0 && $result['offset'] < $result['total']) {
                ...
                $this->redirect($this->url() . '?action=import-contacts&import-filename=' . $fileName . '&import-offset='
        ...
        return function ($viewName, $view) {
            ...
            if ($column && $column->widget->getType() === 'select') {
                ...
                $where = [
                    ...
                    new DataBaseWhere('profile', 'contacts')
                ];
    ...
    public function uploadContactsAction(): Closure
    {
        return function () {
            ...
            $this->redirect($this->url() . '?action=import-contacts&import-filename=' . urlencode($fileName)
