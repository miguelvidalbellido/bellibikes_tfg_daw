...
use FacturaScripts\Plugins\CSVimport\Lib\ImportProfile\ProductsProfile;
use FacturaScripts\Plugins\CSVimport\Lib\ImportProfile\ProfileClass;
use FacturaScripts\Plugins\CSVimport\Lib\ImportProfile\VariantsProfile;
...
class ListProducto
{
    public function createViews(): Closure
    {
        return function () {
            ...
            if ($this->permissions->allowImport) {
                ...
                $this->addButton('ListVariante', [
                    'action' => 'upload-variants',
                    'icon' => 'fas fa-file-import',
                    'label' => 'import',
                    'type' => 'modal'
                ]);
        ...
        return function ($action) {
            switch ($action) {
                ...
                case 'import-variants':
                    $this->importVariantsAction();
                    break;

                case 'upload-products':
                    $this->uploadProductsAction();
                    break;

                case 'upload-variants':
                    $this->uploadVariantsAction();
                    break;
            }
        };
    }

    public function importProductsAction(): Closure
    {
        return function () {
            // obtenemos la ruta completa del archivo
            $fileName = $this->request->get('import-filename');
            $filePath = ProductsProfile::getFilePath($fileName);
            if (empty($filePath)) {
                return true;
            }
            ...
            if ($template === 'new-template') {
                $newCsvFile = CSVfile::newTemplate($fileName, 'products');
            ...
            $templateModel = $template === 'automatic' ?
                ProductsProfile::getFileTemplate($filePath) :
                ProductsProfile::getFileTemplate($filePath, $template);
            ...
            if ($result['offset'] > 0 && $result['offset'] < $result['total']) {
                ...
                $this->redirect($this->url() . '?action=import-products&import-filename=' . $fileName . '&import-offset='
    ...
    public function importVariantsAction(): Closure
    {
        return function () {
            // obtenemos la ruta completa del archivo
            $fileName = $this->request->get('import-filename');
            $filePath = VariantsProfile::getFilePath($fileName);
            if (empty($filePath)) {
                return true;
            }
            ...
            $template = $this->request->get('import-template', 'new-template');
            if ($template === 'new-template') {
                $newCsvFile = CSVfile::newTemplate($fileName, 'variants');
                $newCsvFile->mode = $this->request->get('import-mode');
                if ($newCsvFile->save()) {
                    $this->redirect($newCsvFile->url());
                }
            ...
            // seleccionamos la plantilla
            $templateModel = $template === 'automatic' ?
                VariantsProfile::getFileTemplate($filePath) :
                VariantsProfile::getFileTemplate($filePath, $template);
            if (is_null($templateModel)) {
                ToolBox::i18nLog()->warning('template-not-found');
            ...
            if ($result['offset'] > 0 && $result['offset'] < $result['total']) {
                ...
                ToolBox::i18nLog()->notice('importing');
                $this->redirect($this->url() . '?action=import-variants&import-filename=' . $fileName . '&import-offset='
                    . $result['offset'] . '&save-lines=' . $result['save'] . '&import-template=' . $templateModel->id
                    . '&import-mode=' . $mode, 1);
                return true;
            }
    ...
    public function loadData(): Closure
    {
        return function ($viewName, $view) {
            if ($viewName !== 'ListProducto' && $viewName !== 'ListVariante') {
                return;
            }
            ...
            if ($column && $column->widget->getType() === 'select') {
                ...
                if ($viewName === 'ListProducto') {
                    $where[] = new DataBaseWhere('profile', 'products');
                } elseif ($viewName === 'ListVariante') {
                    $where[] = new DataBaseWhere('profile', 'variants');
                }
    ...
    public function uploadProductsAction(): Closure
    {
        return function () {
            ...
            $this->redirect($this->url() . '?action=import-products&import-filename=' . urlencode($fileName)
                . '&import-template=' . $template . '&import-mode=' . $mode);
            return true;
        };
    }

    public function uploadVariantsAction(): Closure
    {
        return function () {
            // comprobamos los permisos de importación
            if (false === $this->permissions->allowImport) {
            ...
            // comprobamos el tamaño y tipo del archivo
            $uploadFile = $this->request->files->get('variantsfile');
            if (CsvFileTools::isBigFile($uploadFile) || false === CsvFileTools::isValidFile($uploadFile->getRealPath())) {
                return true;
            }
            ...
            $template = $this->request->request->get('template', ProfileClass::AUTOMATIC);
            $mode = $this->request->request->get('mode', ProfileClass::AUTOMATIC);
            $this->redirect($this->url() . '?action=import-variants&import-filename=' . urlencode($fileName)
                . '&import-template=' . $template . '&import-mode=' . $mode);
            return true;
        };
    }
