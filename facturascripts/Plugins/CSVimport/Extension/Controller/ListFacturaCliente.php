...
use FacturaScripts\Plugins\CSVimport\Lib\ImportProfile\CustomerInvoicesProfile;
...
class ListFacturaCliente
{
        ...
        return function ($action) {
            switch ($action) {
                ...
                case 'upload-invoices':
                    $this->uploadInvoicesAction();
                    break;
            }
        };
    }

    public function importInvoicesAction(): Closure
    {
        return function () {
            // obtenemos la ruta completa del archivo
            $fileName = $this->request->get('import-filename');
            $filePath = CustomerInvoicesProfile::getFilePath($fileName);
            if (empty($filePath)) {
                return true;
            }
            ...
            if ($template === 'new-template') {
                $newCsvFile = CSVfile::newTemplate($fileName, 'customer-invoices');
            ...
            // se ha elegido una plantilla existente o automática
            $templateModel = $template === 'automatic' ?
                CustomerInvoicesProfile::getFileTemplate($filePath) :
                CustomerInvoicesProfile::getFileTemplate($filePath, $template);
            ...
            if ($result['offset'] > 0 && $result['offset'] < $result['total']) {
                ...
                $this->redirect($this->url() . '?action=import-invoices&import-filename=' . $fileName . '&import-offset='
        ...
        return function ($viewName, $view) {
            ...
            if ($column && $column->widget->getType() === 'select') {
                ...
                $where = [
                    ...
                    new DataBaseWhere('profile', 'customer-invoices')
                ];
    ...
    public function uploadInvoicesAction(): Closure
    {
        return function () {
            ...
            $this->redirect($this->url() . '?action=import-invoices&import-filename=' . urlencode($fileName)
