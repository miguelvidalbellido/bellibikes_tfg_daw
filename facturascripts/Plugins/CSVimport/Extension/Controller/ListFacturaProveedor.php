...
use FacturaScripts\Plugins\CSVimport\Lib\ImportProfile\SupplierInvoicesProfile;
...
class ListFacturaProveedor
{
    ...
    public function importInvoicesAction(): Closure
    {
        return function () {
            ...
            $filePath = SupplierInvoicesProfile::getFilePath($fileName);
            if (empty($filePath)) {
                return true;
            }
            ...
            if ($template === 'new-template') {
                $newCsvFile = CSVfile::newTemplate($fileName, 'supplier-invoices');
            ...
            $templateModel = $template === 'automatic' ?
                SupplierInvoicesProfile::getFileTemplate($filePath) :
                SupplierInvoicesProfile::getFileTemplate($filePath, $template);
        ...
        return function ($viewName, $view) {
            ...
            if ($column && $column->widget->getType() === 'select') {
                ...
                $where = [
                    ...
                    new DataBaseWhere('profile', 'supplier-invoices')
                ];
