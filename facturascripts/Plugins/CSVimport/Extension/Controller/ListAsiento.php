...
use FacturaScripts\Plugins\CSVimport\Lib\ImportProfile\AccountingEntriesProfile;
...
class ListAsiento
{
        ...
        return function () {
            ...
            if ($this->permissions->allowImport) {
                $this->addButton('ListAsiento', [
                    'action' => 'upload-accounting-entries',
    ...
    public function execPreviousAction(): Closure
    {
        return function ($action) {
            switch ($action) {
                case 'upload-accounting-entries':
                    $this->uploadAccountingEntriesAction();
                    break;

                case 'import-accounting-entries':
                    $this->importAccountingEntriesAction();
                    break;
            }
        };
    }

    public function importAccountingEntriesAction(): Closure
    {
        return function () {
            // obtenemos la ruta completa del archivo
            $fileName = $this->request->get('import-filename');
            $filePath = AccountingEntriesProfile::getFilePath($fileName);
            if (empty($filePath)) {
                return true;
            }
            ...
            if ($template === 'new-template') {
                $newCsvFile = CSVfile::newTemplate($fileName, 'accounting-entries');
            ...
            $templateModel = $template === 'automatic' ?
                AccountingEntriesProfile::getFileTemplate($filePath) :
                AccountingEntriesProfile::getFileTemplate($filePath, $template);
            ...
            if ($result['offset'] > 0 && $result['offset'] < $result['total']) {
                ...
                $this->redirect($this->url() . '?action=import-accounting-entries&import-filename=' . $fileName . '&import-offset='
        ...
        return function ($viewName, $view) {
            ...
            if ($column && $column->widget->getType() === 'select') {
                ...
                $where = [
                    ...
                    new DataBaseWhere('profile', 'accounting-entries')
                ];
    ...
    public function uploadAccountingEntriesAction(): Closure
    {
        return function () {
            ...
            $uploadFile = $this->request->files->get('accountingentriesfile');
            ...
            $this->redirect($this->url() . '?action=import-accounting-entries&import-filename=' . urlencode($fileName)
